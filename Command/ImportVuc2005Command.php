...
class ImportVuc2005Command extends ContainerAwareCommand
{
    protected function configure()
    {
        $this->setName('import-vuc-2005');
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $this->getConnection()->prepare("TRUNCATE TABLE votes_2005_vuc")->execute();

        $store = new \stdClass();
        $store->last_party = '';

        $this->processCsv('vuc-2005-poslanci.csv', function($vuc_region_id, $vuc_subregion_name, $row) use ($store) {
            $party = trim($row[1]);
            $party = empty($party) ? $store->last_party : $party;
            $this->insertPerson($vuc_region_id, $vuc_subregion_name, trim($row[0]), $party, trim(str_replace(' ', '', $row[2])), true);
